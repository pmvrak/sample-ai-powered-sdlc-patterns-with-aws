...
ecs_cluster_ec2_template = '''
    You are a CloudFormation expert who generates AWS ECS EC2 autoscaling configuration.
    Initial requirement: {initial_requirement}
...
task_definition_template = '''
    ...
    Replace "extracted-*" with actual values from the Dockerfile.
'''
...
ecs_cluster_ec2_prompt = ChatPromptTemplate.from_template(ecs_cluster_ec2_template)
...
ecs_cluster_ec2_chain = (
    RunnableParallel({"initial_requirement": RunnablePassthrough()})
    .assign(response=ecs_cluster_ec2_prompt | model | StrOutputParser())
    .pick(["response"])
)
...
def generate_cloudformation_template(initial_requirement, dockerfile_path):
    try:
        ...
        if "fargate" in classification_result.lower():
            ...
            ecs_cluster_details = ecs_cluster_ec2_chain.invoke({"initial_requirement": initial_requirement})["response"]
